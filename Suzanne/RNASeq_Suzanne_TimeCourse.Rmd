...
```{r calling_log2RKPM_data_sets, cache=TRUE}
...
```{r Rep1, results='hide', cache=FALSE}
...
p <- ggplot()+geom_boxplot(data=timecouse_dt, aes(x=Cond, y=RPKM, color=Cond, fill=Cond))+facet_grid(.~Day)
...
p <- ggplot()+geom_boxplot(data=timecouse_dt, aes(x=Cond, y=RPKM, color=Cond, fill=Cond))+geom_boxplot(data=timecouseT0_dt, aes(x=Day, y=RPKM, color=Day, fill=Day))+facet_grid(.~Day)
p+geom_text(data=timecourse_key, aes(x=Cond, y=RPKM, label=ID))+geom_text(data=timecourseT0_key, aes(x=Day, y=RPKM, label=ID))
```
## Visualization for dosage data set
```{r boxplots_Dx_dosage_data, cache=FALSE}
...
p <- ggplot()+geom_boxplot(data=Dosage_dt, aes(x=Dosage, y=RPKM, color=Dosage, fill=Dosage))+geom_boxplot(data=Dosage_dtJx, aes(x=Cond, y=RPKM, color=Cond, fill=Cond))#+facet_grid(.~Dosage)+theme_bw()
p+geom_text(data=Dosage_key, aes(x=Dosage, y=RPKM, label=ID))+geom_text(data=Dosage_keyJx, aes(x=Cond, y=RPKM, label=ID))

```
# Visualization for DN dat sets
```{r boxplots_DN_data, results='asis', cache=FALSE}
